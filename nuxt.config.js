...
const config = {
  ...
  generate: {
    routes () {
      return Promise.all([
        ...
        cdaClient.getEntries({
            'content_type': ctfConfig.CTF_EVENTS_TYPE_ID
          }),
          // get the blog post content type
        cmaClient.getSpace(ctfConfig.CTF_SPACE_ID)
            .then(space => space.getContentType(ctfConfig.CTF_EVENTS_TYPE_ID))
      ])
      .then(([entries, postType, eventsEntries, eventsPostType]) => {
        return [
          // map entries to URLs
          ...
          ...postType.fields.find(field => field.id === 'tags').items.validations[0].in.map(tag => `/tags/${tag}`),
           // map entries to URLs
           ...eventsEntries.items.map(eventEntry => `/blog/${eventEntry.fields.slug}`),
           // map all possible tags to URLs
           ...eventsPostType.fields.find(field => field.id === 'tags').items.validations[0].in.map(tag => `/tags/${tag}`)
        ]
      })
    }
  },
  ...
  */
  env: {
    ...
    CTF_EVENTS_TYPE_ID: ctfConfig.CTF_EVENTS_TYPE_ID
  }
}
